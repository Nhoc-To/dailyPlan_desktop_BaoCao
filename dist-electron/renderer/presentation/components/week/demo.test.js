...
const makeTask = (id, startDate, endDate, completedDays = '[]') => ({
    id,
    categoryId: 1,
    name: `Task ${id}`,
    startDate,
...
test('task 3 ngày ánh xạ đúng vào từng ngày trong khoảng, không xuất hiện ngoài biên', () => {
    ...
    const map = (0, weeklyHelpers_1.buildTasksByDateStr)([task]);
...
test('task cùng id không bị thêm trùng khi truyền 2 lần', () => {
    ...
    const map = (0, weeklyHelpers_1.buildTasksByDateStr)([task, task]);
...
test('addDaysToStr cộng ngày qua ranh giới tháng và năm nhuận chính xác', () => {
    expect((0, weeklyHelpers_1.addDaysToStr)('2025-01-31', 1)).toBe('2025-02-01');
    expect((0, weeklyHelpers_1.addDaysToStr)('2025-02-28', 1)).toBe('2025-03-01');
    expect((0, weeklyHelpers_1.addDaysToStr)('2024-02-28', 1)).toBe('2024-02-29'); // năm nhuận
    expect((0, weeklyHelpers_1.addDaysToStr)('2024-12-31', 1)).toBe('2025-01-01');
});
// Test 4: formatDateStr padding số 0 đúng ở tháng và ngày đơn chữ số
test('formatDateStr tháng và ngày luôn có 2 chữ số', () => {
    expect((0, weeklyHelpers_1.formatDateStr)(new Date(2025, 0, 5))).toBe('2025-01-05');
    expect((0, weeklyHelpers_1.formatDateStr)(new Date(2025, 8, 1))).toBe('2025-09-01');
    expect((0, weeklyHelpers_1.formatDateStr)(new Date(2025, 11, 31))).toBe('2025-12-31');
});
...
test('isTaskCompletedOnDate trả về false khi completedDays là JSON không hợp lệ', () => {
    ...
    expect((0, weeklyHelpers_1.isTaskCompletedOnDate)(task, '2025-01-13')).toBe(false);
});
