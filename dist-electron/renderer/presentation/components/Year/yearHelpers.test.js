...
const yearHelpers_1 = require("./yearHelpers");
// ─── Helper tạo task nhanh ────────────────────────────────────────────────────
const t = (id, startDate, endDate, extra = {}) => ({
    id,
    categoryId: 1,
    name: `Task ${id}`,
    status: false,
...
describe('isBottom', () => {
    test('task status=true → luôn là bottom dù deadline còn xa', () => {
        ...
        expect((0, yearHelpers_1.isBottom)(task, NOW)).toBe(true);
    });
    test('task chưa xong, deadline tương lai → KHÔNG phải bottom', () => {
        const task = t(2, '2026-01-10', '2026-02-01');
        expect((0, yearHelpers_1.isBottom)(task, NOW)).toBe(false);
    });
...
describe('sortTasks', () => {
    ...
    test('sortBy=date: active trước, trong active → startDate mới nhất lên đầu', () => {
        const result = (0, yearHelpers_1.sortTasks)([a1, a2, done, overdue], 'date', NOW);
    ...
    test('sortBy=name: active sắp A–Z, bottom vẫn ở cuối', () => {
        const result = (0, yearHelpers_1.sortTasks)([a1, a2, done, overdue], 'name', NOW);
    ...
    test('sortBy=duration: active có deadline gần nhất lên đầu', () => {
        ...
        const result = (0, yearHelpers_1.sortTasks)([a2, a1], 'duration', NOW);
    ...
    test('không làm biến đổi mảng gốc', () => {
        ...
        (0, yearHelpers_1.sortTasks)(arr, 'date', NOW);
        expect(arr.map(x => x.id)).toEqual(before);
    });
    test('mảng rỗng → trả về mảng rỗng', () => {
        expect((0, yearHelpers_1.sortTasks)([], 'date', NOW)).toEqual([]);
    });
});
// ═══════════════════════════════════════════════════════════════════════════════
...
describe('buildMonthStats', () => {
    test('đếm đúng số task ở từng tháng', () => {
        ...
        const stats = (0, yearHelpers_1.buildMonthStats)(tasks);
    ...
    test('mảng rỗng → 12 số 0', () => {
        expect((0, yearHelpers_1.buildMonthStats)([])).toEqual(Array(12).fill(0));
    });
    test('tổng tất cả tháng = tổng số task', () => {
        ...
        const total = (0, yearHelpers_1.buildMonthStats)(tasks).reduce((s, n) => s + n, 0);
        expect(total).toBe(tasks.length);
    });
    test('kết quả luôn có đúng 12 phần tử', () => {
        expect((0, yearHelpers_1.buildMonthStats)([t(1, '2026-05-01', '2026-05-31')])).toHaveLength(12);
    });
    test('task tháng 12 (index 11) không bị tràn sang tháng 0', () => {
        const stats = (0, yearHelpers_1.buildMonthStats)([t(1, '2026-12-31', '2026-12-31')]);
...
describe('calcSummary', () => {
    test('mảng rỗng → tất cả trường = 0', () => {
        expect((0, yearHelpers_1.calcSummary)([], NOW)).toEqual({ total: 0, completed: 0, overdue: 0, active: 0 });
    });
    test('tính đúng cả 4 nhóm với hỗn hợp task', () => {
        ...
        expect((0, yearHelpers_1.calcSummary)(tasks, NOW)).toEqual({ total: 4, completed: 1, overdue: 1, active: 2 });
    });
    test('task status=true KHÔNG bị đếm vào overdue dù deadline đã qua', () => {
        const task = t(1, '2025-01-01', '2025-01-01', { status: true });
        const s = (0, yearHelpers_1.calcSummary)([task], NOW);
    ...
    test('active = total - completed - overdue luôn đúng', () => {
        ...
        const s = (0, yearHelpers_1.calcSummary)(tasks, NOW);
